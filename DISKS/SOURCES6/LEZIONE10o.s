...
In questo esempio vediamo il funzionamento del fill-mode. Innanzitutto
tracciamo (con il blitter) delle linee sullo schermo. Poi copiamo il rettangolo
di schermo che le racchiude, effettuando il riempimento. Come potete osservare
le aree delimitate dalle linee vengono riempite.
La routine che si occupa del riempimento, legge il rettangolo sorgente da una
posizione fissa dello schermo (dove abbiamo tracciato inizialmente le linee)
mentre la destinazione e` variabile e viene passata attraverso un parametro.
Le dimensioni del rettangolo sono fisse e sono state calcolate in modo da
racchiudere tutte le linee tracciate.
La routine di Fill inoltre e` parametrizzata in modo da effettuare tutti i
tipi di fill: inclusivo, esclusivo, e con FILL_CARRYIN a 0 oppure a 1.
Osservando attentamente le immagini prodotte (sopratutto il rettangolo)
vedrete le differenze tra i tipi di fill.
